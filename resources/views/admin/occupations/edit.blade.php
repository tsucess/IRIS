<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-dark leading-tight">{{ __('Edit Occupation') }}</h2>
    </x-slot>

    <div class="min-h-screen bg-gradient-to-br from-blue-900/60 via-indigo-900/40 to-purple-800/60 text-dark p-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <x-glass-form method="POST" action="{{ route('admin.occupations.update', $occupation) }}">
                @csrf @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <x-text-input id="name" name="name" type="text"
                        ...
                        required :value="old('name', $occupation->name)" />
                ...
                <div class="mb-4">
                    ...
                    <x-text-input id="category" name="category" type="text"
                        class="mt-1 block w-full text-gray-900 rounded-md shadow-sm"
                        :value="old('category', $occupation->category)" />
                    <x-input-error :messages="$errors->get('category')" class="mt-2 text-red-300" />
                </div>

                <div class="mb-4">
                    <x-input-label for="sector" :value="__('Sector')" class="text-white" />
                    <x-text-input id="sector" name="sector" type="text"
                        class="mt-1 block w-full text-gray-900 rounded-md shadow-sm"
                        :value="old('sector', $occupation->sector)" />
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="mt-1 block w-full rounded-md shadow-sm border-gray-300 text-gray-900">{{ old('description', $occupation->description) }}</textarea>
                ...
                <div class="mb-4 flex items-center gap-2">
                    ...
                    <input id="is_active" name="is_active" type="checkbox" value="1"
                        {{ old('is_active', $occupation->is_active) ? 'checked' : '' }} class="rounded" />
                    <label for="is_active" class="text-white">Active</label>
                </div>

                <div class="flex items-center gap-4">
                    <x-primary-button class="bg-blue-600 hover:bg-blue-700 shadow-lg">{{ __('💾 Update') }}</x-primary-button>
