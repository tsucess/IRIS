<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-dark leading-tight">
            {{ __('Edit Allocation · ') . $project->title }}
        </h2>
    </x-slot>

    <div class="min-h-screen bg-gradient-to-br from-blue-900/60 via-indigo-900/40 to-purple-800/60 text-dark p-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <x-glass-form method="POST" action="{{ route('projects.allocations.update', [$project, $allocation]) }}">
                @csrf @method('PUT')
                @include('allocations.partials.form', ['allocation' => $allocation])
